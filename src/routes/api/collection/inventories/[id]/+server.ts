...
async function getInventoryMinifigInventoryParts(inventoryId: string) {
	...
	return inventoryMinifigs.flatMap((inventoryMinifig) => {
		const inventory = inventories.find(
			(inventory) => inventory.setId === inventoryMinifig.minifigId
		);

		if (!inventory) return [];

		const currentInventoryParts = inventoryParts.filter(
			(inventoryPart) => inventoryPart.inventoryId === inventory._id
		);
...
export const POST: RequestHandler = withRouteAuthRequired(async ({ params, locals }) => {
	...
	const deduplicatedInventoryParts = deduplicateInventoryParts([
		...inventoryParts,
		...minifigInventoryParts
	]);
