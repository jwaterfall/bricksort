...
import { type Theme, ThemeModel } from '../models/theme';
import { type Color, ColorModel } from '../models/color';
import { type PartCategory, PartCategoryModel } from '../models/part-category';
import { type Part, PartModel } from '../models/part';
import {
  type PartRelationship,
  PartRelationshipModel,
} from '../models/part-relationship';
import { type Set, SetModel } from '../models/set';
import { type Element, ElementModel } from '../models/element';
import { type Minifig, MinifigModel } from '../models/minifig';
import { type Inventory, InventoryModel } from '../models/inventory';
import {
  type InventoryPart,
  InventoryPartModel,
} from '../models/inventory-part';
import { type InventorySet, InventorySetModel } from '../models/inventory-set';
import {
  type InventoryMinifig,
  InventoryMinifigModel,
} from '../models/inventory-minifig';
...
const getProgressBar = (total: number, current: number) => {
  ...
  return `${chalk.green('='.repeat(bars))}${' '.repeat(
    PROGRESS_BAR_LENGTH - bars
...
const syncPartCategories: SyncFunction = async () => {
  ...
  partCategories.forEach((partCategory, index) => {
    ...
    if (index % 100 === 0) {
      console.log(
        'Part Categories: ',
        getProgressBar(partCategories.length, index)
      );
    }
  });
...
const syncPartRelationships: SyncFunction = async () => {
  ...
  partRelationships.forEach((partRelationship, index) => {
    ...
    if (index % 100 === 0) {
      console.log(
        'Part Relationships: ',
        getProgressBar(partRelationships.length, index)
      );
    }
  });
...
const syncInventoryParts: SyncFunction = async () => {
  ...
  inventoryParts.forEach((inventoryPart, index) => {
    ...
    if (index % 1000 === 0) {
      console.log(
        'Inventory Parts: ',
        getProgressBar(inventoryParts.length, index)
      );
    }
  });
...
const syncInventorySets: SyncFunction = async () => {
  ...
  inventorySets.forEach((inventorySet, index) => {
    ...
    if (index % 100 === 0) {
      console.log(
        'Inventory Sets: ',
        getProgressBar(inventorySets.length, index)
      );
    }
  });
...
const syncInventoryMinifigs: SyncFunction = async () => {
  ...
  inventoryMinifigs.forEach((inventoryMinifig, index) => {
    ...
    if (index % 100 === 0) {
      console.log(
        'Inventory Minifigs: ',
        getProgressBar(inventoryMinifigs.length, index)
      );
    }
  });
...
const main = async () => {
  ...
  for (const syncFunction of syncFunctions) {
    ...
    try {
      ...
      const end = Date.now();
      console.log(
        `- ${result.name}: ${result.count} records in ${end - start}ms`
      );
    } catch (error) {
